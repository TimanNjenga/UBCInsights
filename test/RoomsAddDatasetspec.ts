...
describe("RoomsAddDatasetspec", function () {

    let insightfacade : InsightFacade  = null;

    let fs = require('fs');
    let astring = "rooms";
    let AnotherString = "buildings-and-classrooms" ;
    let data = fs.readFileSync('rooms.zip');
    let contentstring = data.toString('base64');
    let data2 = fs.readFileSync('testRooms.zip');
    ...
    it("should return new saved file ",function(){
    ...
    it("should return second new correctly saved file ",function(){
        return insightfacade.addDataset(astring , contentstring2)
            .then(function (response: InsightResponse) {
                Log.test('Response number: ' + response.code);
                Log.test('Response body: ' + response.body);
                expect(response.code).to.deep.equal(204);
